...
class EMLParser {
    ...
    async parse(file) {
        try {
            ...
            if (separatorIndex === -1) {
                console.error('No header-body separator found');
                throw new Error('Invalid email format');
            }

            console.log('Found header-body separator at:', separatorIndex);
            ...
            console.log('Parsed body length:', this.processedBody?.length);
            console.log('Found attachments:', this.attachments.length);
            console.log('Found inline images:', this.inlineImages.size);
    ...
    async parseBody(content) {
        console.log('Starting body parsing, content length:', content.length);
        console.log('Number of boundaries:', this.boundaries.length);

        if (this.boundaries.length > 0) {
            // Multipart email
            console.log('Parsing multipart email');
            ...
            for (const part of parts) {
                ...
                console.log('Part headers:', partHeaders);
                console.log('Part content length:', partContent.length);
            ...
            console.log('Processed body length:', this.processedBody.length);
        } else {
            // Single part email
            console.log('Parsing single part email');
        ...
        console.log('Body parsing complete');
    }
    ...
    async decodeContent(content, headers) {
        console.log('Decoding content with encoding:', headers['content-transfer-encoding']);
        ...
        if (charset && charset !== 'utf-8') {
            try {
                ...
                console.warn('Charset conversion failed:', e);
            }
        }
...
class UIHandler {
    ...
    displayEmail(result) {
        ...
        // Log for debugging
        console.log('Email display updated:', result);
    }
